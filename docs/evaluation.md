...
若要在同一套任务和推理参数下依次评测 SFT 的多个课程阶段以及一个 GRPO 模型，使用
[`scripts/evaluate_sft_grpo.py`](../scripts/evaluate_sft_grpo.py)。脚本只启动一次
ShopSimulator，每个模型单独启动并停止 vLLM；`evaluate_shop_benchmark.py` 会按已有
`trajectories.jsonl` 断点续跑。

模型已经是可直接 serve 的 Hugging Face 目录时：

```bash
.venv/bin/python scripts/evaluate_sft_grpo.py \
  --sft-root /data/shopping-rl/models/sft-curriculum \
  --grpo-model /data/shopping-rl/models/grpo-merged-step100 \
  --benchmark data/grpo/validation.jsonl \
  --output-root outputs/evaluation/sft-grpo-step100
```

如果 GRPO 仍是 veRL actor checkpoint，明确指定要导出的 step 和导出目录：

```bash
.venv/bin/python scripts/evaluate_sft_grpo.py \
  --sft-root /data/shopping-rl/models/sft-curriculum \
  --grpo-actor /data/shopping-rl/models/grpo-qwen35-2b/global_step_100/actor \
  --grpo-export /data/shopping-rl/models/grpo-merged-step100 \
  --benchmark data/grpo/validation.jsonl
```

运行前可用 `--dry-run` 检查四个模型目录和最终命令。正式 Final-200 应在 checkpoint
选择和训练配方冻结后再运行；日常 checkpoint 对比优先使用 GRPO validation 或独立
开发集，避免用盲测集选择模型。输出目录包含每个模型的 `summary.json`、
`trajectories.jsonl`、`report.html`，以及统一的 `comparison.md/json`。
