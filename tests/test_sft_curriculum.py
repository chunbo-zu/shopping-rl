...
class SftCurriculumTest(unittest.TestCase):
    ...
    def test_four_gpu_ddp_preserves_fixed_effective_batch_size(self):
        manifest = {
            "stages": {
                stage: {"epochs": 1.0, "learning_rate": 1e-4}
                for stage in ("a", "b", "c")
        ...
        self.assertEqual(
            ...
            [
                ...
                "--standalone",
                "--nproc-per-node=4",
                str(Path(__file__).resolve().parents[1] / "scripts/train_lora_sft.py"),
            ],
        )
        accumulation = train.index("--gradient-accumulation-steps")
        self.assertEqual(train[accumulation + 1], "2")
