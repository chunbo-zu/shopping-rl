...
class DynamicSamplingConfigTest(unittest.TestCase):
    ...
    def test_prompt_length_filter_runs_in_process(self):
        config = compose_runtime_config([])
        self.assertTrue(config.data.filter_overlong_prompts)
    ...
    def test_training_memory_budget_rejects_unsafe_overrides(self):
        ...
        unused_entropy = compose_runtime_config(
            ["actor_rollout_ref.actor.calculate_entropy=true"]
        )
        with self.assertRaisesRegex(
            SystemExit, "calculate_entropy must be false"
        ):
            validate_training_memory_budget(unused_entropy)
