...
def main():
    ...
    # In DDP every worker executes this file.  Only rank zero may create the
    # explicit SwanLab run; the Transformers callback uses the same main-rank
    # convention for subsequent logging.
    is_global_rank_zero = int(os.environ.get("RANK", "0")) == 0
    if report_to == "swanlab" and is_global_rank_zero:
        import swanlab
        swanlab.init(
