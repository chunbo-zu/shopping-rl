...
MAX_SAFE_RESPONSE_LENGTH = 8192
MAX_SAFE_SEQUENCE_LENGTH = 12288
MODEL_CONTEXT_LENGTH = 24576
...
def validate_trace(config):
    ...
    if int(trace.get("max_sequence_length", 0)) != MODEL_CONTEXT_LENGTH:
        raise SystemExit(
            f"shopping TRACE max_sequence_length must equal {MODEL_CONTEXT_LENGTH}"
        )


def validate_sao(config):
    """Validate the independent Single-Rollout Async Optimization arm."""
    sao = config.get("shopping_sao", {})
    if not bool(sao.get("enable", False)):
        return

    if str(config["algorithm"]["adv_estimator"]).lower() != "gae":
        raise SystemExit("shopping SAO requires algorithm.adv_estimator=gae")
    rollout = config["actor_rollout_ref"]["rollout"]
    if str(rollout.get("mode", "")).lower() != "async":
        raise SystemExit("shopping SAO requires asynchronous rollout mode")
    if int(rollout.get("n", 0)) != 1 or not bool(sao.get("single_rollout", False)):
        raise SystemExit("shopping SAO requires exactly one rollout per prompt")
    if not bool(rollout.get("calculate_log_probs", False)):
        raise SystemExit("shopping SAO requires rollout.calculate_log_probs=true")

    correction = config["algorithm"].get("rollout_correction", {})
    if not bool(correction.get("bypass_mode", False)):
        raise SystemExit("shopping SAO requires rollout_correction.bypass_mode=true")
    if not (
        bool(correction.get("sao_dis", False))
        or correction.get("loss_type") == "sao_dis"
    ):
        raise SystemExit(
            "shopping SAO requires rollout_correction.loss_type=sao_dis"
        )

    if bool(config.get("shopping_dynamic_sampling", {}).get("enable", False)):
        raise SystemExit("shopping SAO does not combine with dynamic group sampling")
    if bool(config.get("shopping_trace", {}).get("enable", False)):
        raise SystemExit("shopping SAO does not combine with shopping TRACE")
    if not bool(config.get("critic", {}).get("enable", False)):
        raise SystemExit("shopping SAO requires critic.enable=true for GAE")
    if not bool(sao.get("skip_observation_gae", False)):
        raise SystemExit("shopping SAO requires skip_observation_gae=true")
    if not bool(sao.get("freeze_critic_attention", False)):
        raise SystemExit("shopping SAO requires freeze_critic_attention=true")

    low_clip = float(
        correction.get("sao_low_clip", sao.get("low_clip", -1))
    )
    high_clip = float(
        correction.get("sao_high_clip", sao.get("high_clip", -1))
    )
    if not math.isfinite(low_clip) or not math.isfinite(high_clip):
        raise SystemExit("shopping SAO DIS clip bounds must be finite")
    if not 0 <= low_clip < 1 or high_clip < 0:
        raise SystemExit("shopping SAO DIS clip bounds must satisfy low in [0,1), high >= 0")

    actor_epochs = int(config["actor_rollout_ref"]["actor"].get("ppo_epochs", 1))
    critic_epochs = int(config["critic"].get("ppo_epochs", 0))
    requested_critic_epochs = int(sao.get("critic_epochs_per_actor", critic_epochs))
    if actor_epochs <= 0 or critic_epochs < requested_critic_epochs or requested_critic_epochs <= 0:
        raise SystemExit(
            "shopping SAO requires critic.ppo_epochs >= critic_epochs_per_actor > 0"
        )

    print(
        "shopping SAO preflight passed: "
        + json.dumps(
            {
                "enable": True,
                "rollout_n": int(rollout.n),
                "rollout_mode": str(rollout.mode),
                "direct_double_sided_is": True,
                "low_clip": low_clip,
                "high_clip": high_clip,
                "skip_observation_gae": True,
...
def validate_training_memory_budget(config):
    ...
    if bool(actor.calculate_entropy) and float(actor.entropy_coeff) == 0.0:
        raise SystemExit(
            "actor.calculate_entropy must be false when actor.entropy_coeff is zero; "
            "Qwen3.5 full-vocabulary entropy retains an unnecessary multi-GiB "
            "autograd graph"
        )
    for name, value in (
        ...
        if value != MODEL_CONTEXT_LENGTH:
            raise SystemExit(
                f"unsafe or inconsistent GRPO context budget: {name} must equal "
                f"{MODEL_CONTEXT_LENGTH}, got {value}"
            )
        ...
        + json.dumps(
            {
                ...
                "calculate_entropy": bool(actor.calculate_entropy),
