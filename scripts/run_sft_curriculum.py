...
def build_stage_commands(
    ...
    *,
    ...
    # The fixed recipe has effective batch size 8: per-device batch 1 and
    # gradient accumulation 8 on one GPU.  DDP multiplies the batch by world
    # size, so reduce local accumulation to preserve the optimization recipe.
    if 8 % num_processes:
        raise ValueError("--num-processes must divide the fixed effective batch size 8")
    ...
    for index, stage in enumerate(STAGES[start : stop + 1]):
        ...
        train_entrypoint = [str(python)]
        if num_processes > 1:
            train_entrypoint.extend(
                [
                    "-m",
...
def parse_args():
    ...
    parser.add_argument("--swanlab-mode", choices=("online", "local"), default="online")
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="单机 DDP 进程/GPU 数；必须能整除固定的全局 batch size 8。",
    )
